...
struct DADAC {
    ...
    int hod;
    base_t() : dugc(0), hod(0) {
        std::cout << "base_t constructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
    base_t(int __dugc, int __hod) : dugc(__dugc), hod(__hod) {
        std::cout << "base_t constructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
    virtual ~base_t() {
        std::cout << "base_t destructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
};

struct ADAC : virtual public DADAC {
    int hod;
    int dean;
    derived_t() : base_t(0, 0), hod(0), dean(0) {
        std::cout << "derived_t constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    derived_t(int __hod, int __dean) : base_t(0, 0), hod(__hod), dean(__dean) {
        std::cout << "derived_t constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    derived_t(int __dugc, int __hod_dept, int __hod, int __dean) :
        base_t(__dugc, __hod_dept), hod(__hod), dean(__dean) {
        std::cout << "derived_t constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    ~derived_t() {
        std::cout << "derived_t destructor with hod = " << hod << " dean = " << dean << std::endl;
    }
};
